...
<div id="app" class="fixed top-0 left-0 bottom-0 right-0 bg-black">
    {#key data.pathname}
        <main in:slideIn out:foldOut class="fixed z-10 top-0 left-0 bottom-0 right-0">
            <div id="page" on:scroll={onPageScroll} class="w-full h-full overflow-y-scroll bg-white flex flex-col">
                <Header />
                <Drawer />
                <div class="flex-1">
                    <slot/>
...
<style>
    /* Tailwind doesn't support perspective inline */
    #app {
        perspective: 960px; 
    }
</style>